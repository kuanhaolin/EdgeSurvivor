...
# 測試參數（優化配置 - 快速啟動，充分測試）
START_USERS="${START_USERS:-200}"           # 起始用戶數
INCREMENT="${INCREMENT:-50}"               # 每次增加用戶數
TEST_DURATION="${TEST_DURATION:-30}"       # 每輪測試持續時間（增加到 30 秒）
RAMPUP="${RAMPUP:-15}"                      # 啟動時間（縮短到 15 秒）
MAX_ERROR_RATE="${MAX_ERROR_RATE:-5}"     # 最大錯誤率（%）
MAX_USERS="${MAX_USERS:-3000}"             # 最大用戶數限制（極限測試）
BREAKTIME="${BREAKTIME:-60}"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  登入 API 壓力測試${NC}"
...
while [ $CURRENT_USERS -le $MAX_USERS ]; do
    ...
    if [ $CURRENT_USERS -le $MAX_USERS ]; then
        echo -e "${BLUE}等待 ${BREAKTIME} 秒讓系統和網絡恢復...${NC}"
        sleep "$BREAKTIME"
    fi
done
