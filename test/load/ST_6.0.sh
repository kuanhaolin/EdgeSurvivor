...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  建立活動 API 壓力測試${NC}"
echo -e "${BLUE}  POST /api/activities${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
echo ""
echo -e "${YELLOW}注意：此測試會生成大量活動數據${NC}"
echo ""
...
RESULT_DIR="results/activities-create-stress-$TIMESTAMP"
mkdir -p "$RESULT_DIR"

# 建立統計檔案
STATS_FILE="$RESULT_DIR/test-summary.txt"
echo "建立活動 API 壓力測試報告" > "$STATS_FILE"
