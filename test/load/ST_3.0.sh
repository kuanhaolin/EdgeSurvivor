...
# 預設參數
# HOST="${HOST:-edgesurvivor.ddns.net}"
# PORT="${PORT:-443}"
...
RECEIVER_ID="${RECEIVER_ID:-7}"

# 測試參數
START_USERS="${START_USERS:-500}"         # 起始用戶數
INCREMENT="${INCREMENT:-100}"             # 每次增加用戶數
TEST_DURATION="${TEST_DURATION:-30}"      # 測試持續時間（秒）
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  聊天消息 API 壓力測試${NC}"
echo -e "${BLUE}  POST /api/chat/messages${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
RESULT_DIR="results/chat-message-stress-$TIMESTAMP"
mkdir -p "$RESULT_DIR"

# 建立統計檔案
STATS_FILE="$RESULT_DIR/test-summary.txt"
echo "聊天消息 API 壓力測試報告" > "$STATS_FILE"
echo "測試時間: $(date)" >> "$STATS_FILE"
echo "主機: $PROTOCOL://$HOST:$PORT" >> "$STATS_FILE"
echo "接收者ID: $RECEIVER_ID" >> "$STATS_FILE"
echo "========================================" >> "$STATS_FILE"
echo "" >> "$STATS_FILE"
...
jmeter -n \
  ...
  -Jtoken="$TOKEN" \
  -Jreceiver_id="$RECEIVER_ID" \
  -l "$ROUND_DIR/results.jtl" \
  > "$ROUND_DIR/ST_3.0.log" 2>&1
...
if [ "$TOTAL" -eq 0 ]; then
    ...
    echo "" >> "$STATS_FILE"
    echo "第 $ROUND 輪測試失敗 - 無數據產生" >> "$STATS_FILE"
    echo "JMeter 日誌: $ROUND_DIR/jmeter.log" >> "$STATS_FILE"
    echo "" >> "$STATS_FILE"
    echo -e "${YELLOW}查看 JMeter 日誌: cat $ROUND_DIR/jmeter.log${NC}"
    break
fi

SUCCESS=$(grep ",201," "$ROUND_DIR/results.jtl" | wc -l | tr -d ' ')
