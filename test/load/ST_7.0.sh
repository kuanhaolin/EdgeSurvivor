...
# 測試參數
START_USERS="${START_USERS:-200}"           # 起始用戶數（註冊較慢，從小數字開始）
...
MAX_USERS="${MAX_USERS:-1000}"             # 最大用戶數限制
BREAKTIME="${BREAKTIME:-60}"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  註冊 API 壓力測試${NC}"
...
echo ""
echo -e "${YELLOW}注意：此測試會生成大量隨機用戶數據${NC}"
echo ""
...
RESULT_DIR="results/register-stress-$TIMESTAMP"
mkdir -p "$RESULT_DIR"

# 建立統計檔案
STATS_FILE="$RESULT_DIR/test-summary.txt"
echo "註冊 API 壓力測試報告" > "$STATS_FILE"
